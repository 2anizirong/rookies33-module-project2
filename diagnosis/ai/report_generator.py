...
def generate(
    ...
    """진단 증거 + Web 조사를 종합한다. 이 단계에서는 검색하지 않는다."""
    ...
    prompt = f"""
다음 두 자료를 종합하여 최종 Security Intelligence 보고서를 작성하라.
...
8. IAM Role 이름, AWS Account ID, ARN, Bucket/Lambda 실제 이름, Credential 값 등 구체 식별자/값 문자열은 리포트에 표기하지 않는다(추측·생성도 금지).
9. 단, iam_role_identified / credential_values_captured / resource_names_identified / principal_identified 플래그가 true이면 해당 식별자·자격증명은 진단 과정에서 실제로 확인/획득된 사실이다. 이런 항목은 절대 '미확인'이나 '확보 못함'으로 서술하지 말고 '진단 과정에서 확인됨(구체 식별자·값은 본 리포트에 표기하지 않음)'처럼 확인된 사실로 서술한다. 진단 한계(limitations)에는 실제로 미확인인 항목만 적는다.
10. IMDSv2는 SSRF 자체 제거가 아니라 metadata 접근/credential 탈취에 대한 방어 심층화로 설명한다.
...
현재 진단의 실제 체인을 먼저 설명하고, Web Search는 그 사실을 보강하는 자료로만 사용하라.
